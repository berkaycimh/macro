...
; ── Titlebar (h=44) ──
G.Add("Text", "x0 y0 w4 h44 Background00ff88")
...
G.Add("Text", "x110 y53 w40 Background0a0a0a", "PUBG")
...
G.SetFont("s9 w700 c555555", "Consolas")
btnUp    := G.Add("Text", "x106 y282 w24 h24 Background1a1a1a Center", "▲")
btnLeft  := G.Add("Text", "x132 y282 w24 h24 Background1a1a1a Center", "◄")
btnDown  := G.Add("Text", "x158 y282 w24 h24 Background1a1a1a Center", "▼")
btnRight := G.Add("Text", "x184 y282 w24 h24 Background1a1a1a Center", "►")
G.SetFont("s7 c333333", "Consolas")
...
CheckVersion() {
    ...
    try {
        ...
        if (latestVer != currentVersion) {
            ...
            if (newCode = "")
                return
            tmpFile := A_ScriptDir "\macro_update.ahk"
            FileOpen(tmpFile, "w", "UTF-8").Write(newCode)
            FileCopy(A_ScriptFullPath, A_ScriptDir "\macro_backup.ahk", 1)
            FileOpen(A_ScriptFullPath, "w", "UTF-8").Write(newCode)
...
SaveSettings(reason, code) {
    global recoilValues, activeAmmo, HUD
    global statTotal, stat556, stat762, stat9mm
    iniFile := A_ScriptDir "\settings.ini"
    IniWrite(recoilValues["5.56"], iniFile, "Recoil",   "556")
    IniWrite(recoilValues["7.62"], iniFile, "Recoil",   "762")
